        ...
        .then(
            async response => {
                ...
                if (highest_mayor_votes != mayor_data[0].votes) {
                    ...
                    for (i = 0; i < 3; i++) {
                        mayor_text += `${mayor_data[i].candidate_name}:${Number(mayor_data[i].votes)}(${(Number(mayor_data[i].votes / total_mayor_votes) * 100).toFixed(2)}%)\n`
                        mayor_loop_total += Number(mayor_data[i].votes)
                    }
                    mayor_text += `Others:${total_mayor_votes - mayor_loop_total}:(${(Number(total_mayor_votes - mayor_loop_total) / Number(total_mayor_votes) * 100).toFixed(2)}%)`
